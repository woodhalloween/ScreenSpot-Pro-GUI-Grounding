...
def add_text(state, text, image, image_process_mode, request: gr.Request):
    ...
    text = text[:16384]  # Hard cut-off
    if image is not None:
        text = text[:16384]  # Hard cut-off for images
        if '<image>' not in text:
            # text = '<Image><image></Image>' + text
            text ="In the screenshot, where are the pixel coordinates (x, y) of the element corresponding to "+ f"\"{text}\"?"  + '\n<image>'
...
# UGround: Universal Visual Grounding For GUI Agents
 [[Project Page]](https://osu-nlp-group.github.io/UGround/) [[Paper]](https://github.com/OSU-NLP-Group/UGround/blob/gh-pages/static/papers/UGround_paper.pdf) [[Code]](https://github.com/OSU-NLP-Group/UGround) [[Model]](https://huggingface.co/osunlp/UGround)
""")
        ...
        with gr.Row():
            with gr.Column(scale=3):
                ...
                gr.Examples(examples=[
                    [f"{cur_dir}/amazon.jpg", "Where is the search bar at the top of the page?"],
                ...
                with gr.Accordion("Parameters", open=False) as parameter_row:
                    ...
                    max_output_tokens = gr.Slider(minimum=0, maximum=16384, value=16384, step=64, interactive=True, label="Max output tokens",)
